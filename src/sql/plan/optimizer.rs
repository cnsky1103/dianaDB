    ...
    fn optimize(&self, node: Node) -> Result<Node> {
        node.transform(&|n| Ok(n), &|n| {
            n.transform_expressions(
                &|e| {
                    // if e does not contain a field, which means e's children are constants
                    // we can evaluate e
    ...
    fn pushdown(&self, mut expression: Expression, target: &mut Node) -> Option<Expression> {
        match target {
            Node::Scan { ref mut filter, .. } => {
                // if a filter's source is a scan, we can apply the predicate of the filter
                // to the scan so we push it down
